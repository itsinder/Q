...
\be
\item \(n_l\) --- {\tt nl}, number of layers. The simplest DNN has 3 layers --- 
an input layer, a hidden layer and an output layer.
\item \(p[l]\) -- {\tt npl}, number of neurons per layer. The network of
Figure~\ref{sample_network} would have \(n_l = 3, p = \{3,4,1\}\)
\item \(W\) --- the weights on the edges. 
\(W_i\) contains the edges from layer \(i-1\) to layer \(i\). 
Hence, 
\be
\item \(W_0 = \bot\)
\item \(W_i = \) edges from layer \(i-1\) to layer \(i\)
\item \(W_{i,j} = \) edges from neuron \(j\) of layer \(i-1\) to layer \(i\).
Note that \(j = 0, 1, p_{i-1} -1\).
\item \(W_{i,j,k} = \) edges from neuron \(j\) of layer \(i-1\) to 
neuron \(k\) of layer \(i\).
Note that \(k = 0, 1, p_i -1\).
\ee
