...
int allocate_persistent_data(long long siz, long long min_freq, long long err, long long max_chunk_size, struct frequent_persistent_data *data) {
  int status = 0;
  ...
  packet_space = malloc(cntr_siz * sizeof(int)); return_if_malloc_failed(packet_space);
...
int process_chunk(int *chunk, int chunk_siz, struct frequent_persistent_data *data) {
  int NUM_THREADS = 1;
  int status = 0;
  ...
  for (int tid = 0; tid < NUM_THREADS; tid++) {
    ...
    qsort_asc_I4(packet_starts[tid], packet_sizs[tid], sizeof(int), NULL);
  }
...
int process_output(struct frequent_persistent_data *data, int **y, int **f, int *out_len) {
  int status = 0;
