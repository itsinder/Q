...
function Vector_Wrapper(metadata, chunk_size)
  ...
  self.v =  Vector{field_type=self.data_type, field_size=self.size_of_data, filename= _G["Q_DATA_DIR"] .. "_" ..metadata.name, write_vector=true}
  self.chunk_size = chunk_size or 1 --if value is not specified default to 15
  ...
  if metadata.null == true or metadata.null == "true" then
    ...
    self.nn_v = Vector{field_type="I1", field_size=g_qtypes["I1"]["width"],
       filename= _G["Q_DATA_DIR"] .. "_nn_" ..metadata.name, write_vector=true}       
  end
  
  if(metadata.type == "varchar") then
  ...
  local write = function(data)
    local IS_NULL  = 0; 
    if self.data_type == "varchar" then 
