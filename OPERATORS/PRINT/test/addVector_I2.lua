...
[[ 
  ...
  void free(void *ptr);
  void initialize_vector_I2(int16_t *vec, int n);
  int print_vector(int* ptr, int length); 
]]
local c = ffi.load('print.so')
local lib = ffi.load("initializevector_I2.so")
...
filename="./bin/I2_1.bin", write_vector = true,  
}

local v2 = Vector{field_type='I2', field_size = 2,chunk_size = 6,
filename="./bin/I2_2.bin", write_vector = true,
}
...
x[1] = ffi.cast("int16_t * ", x[1])
x[2] = ffi.cast("int16_t * ", x[2])


lib["initialize_vector_I2"](x[1], x_size[1])
lib["initialize_vector_I2"](x[2], x_size[2])
