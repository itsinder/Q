...
local function maxk_specialize(val_fldtype, drag_fldtype)

  local hdr = [[
  typedef struct _reduce_maxk_<<v_qtype>>_<<d_qtype>>_args {
  <<reduce_v_ctype>> *val; // [k]
  <<reduce_d_ctype>> *drag; // [k]
  uint64_t n; // actual occupancy
  uint64_t k; // max occupancy
  } REDUCE_maxk_<<v_qtype>>_<<d_qtype>>_ARGS;
  ]]
  ...
  local struct_type = "REDUCE_maxk_" .. v_qtype .. "_" .. d_qtype .. "_ARGS"
  ...
  subs.c_mem = c_mem
  subs.c_mem_type = "REDUCE_maxk_" .. v_qtype .. "_" .. d_qtype .. "_ARGS *"

  local tmpl = "getk.tmpl"
  subs.fn = "maxk_" .. v_qtype .. "_" .. d_qtype
  subs.comparator = ">"
  subs.op = "maxk"
