local function vv_ifxthenyelsez(x, y, z)
  ...
  local spfn = require("Q/OPERATORS/IFXTHENYELSEZ/lua/vv_ifxthenyelsez_specialize" )
  assert(type(spfn) == "function")
  assert(x:fldtype() == "B1")
  assert(y:fldtype() == z:fldtype())
  local status, subs, tmpl = pcall(spfn, "vv", y:fldtype())
  if ( not status ) then print(subs) end
  assert(status, "error in call to vv_ifxthenyelsez_specialize")
  assert(type(subs) == "table", "error in call to vv_ifxthenyelsez_specialize")
  local func_name = assert(subs.fn)
  -- allocate buffer for output
  local wbufsz = qconsts.chunk_size * ffi.sizeof(subs.ctype)
  local wbuf = assert(ffi.malloc(wbufsz))
  --
  local function vv_ifxthenyelsez_gen(chunk_idx)
