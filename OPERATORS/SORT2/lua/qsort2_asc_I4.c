...
{ \
  ...
  if ( ( aidx < 0 ) || ( aidx >= total_elems ) ) { WHEREAMI; return -1;  } \
  if ( ( bidx < 0 ) || ( bidx >= total_elems ) ) { WHEREAMI; return -1;  } \
  aval = drag[aidx];                                                \
  bval = drag[bidx];                                                \
  drag[aidx] = bval;                                                \
  drag[bidx] = aval;						\
  ...
  if ( total_elems > MAX_THRESH ) {
    ...
    while (STACK_NOT_EMPTY) {
	 ...
	 the while loops. */
      ...
      if ( *((int32_t *)mid) < *((int32_t *)lo) ) {
	SWAP (mid, lo, size);
      }
      // if ( (*cmp) ((void *) hi, (void *) mid) < 0) {
      if ( *((int32_t *)hi) < *((int32_t *)mid) ) {
	SWAP (mid, hi, size);
      }
      else {
	goto jump_over;
	...
	while ( *((int32_t *)left_ptr) < *((int32_t *)mid) ) {
	  left_ptr += size;
	}

	// while ((*cmp) ((void *) mid, (void *) right_ptr) < 0) {
	while ( *((int32_t *)mid) < *((int32_t *)right_ptr) ) {
	  right_ptr -= size;
	}
...
#define min(x, y) ((x) < (y) ? (x) : (y))

  {
    ...
    for (run_ptr = tmp_ptr + size; run_ptr <= thresh; run_ptr += size) {
      ...
      if (  *((int32_t *)run_ptr) < *((int32_t *) tmp_ptr) ) {
        tmp_ptr = run_ptr;
      }
    }
    ...
    while ((run_ptr += size) <= end_ptr) {
      ...
      while ( *((int32_t *)run_ptr) < *((int32_t *)tmp_ptr) ) {
	tmp_ptr -= size;
      }
