...
#define STACK_SIZE (BYTES_PER_WORD * sizeof (uint64_t))
...
/* Order size using quicksort.  This implementation incorporates
   ...
   3. Only quicksorts nX / MAX_THRESH partitions, leaving
    ...
    )
{
  ...
  int32_t *pivot_buffer = malloc (size);
  int   max_thresh   = MAX_THRESH * size;

  if ( nX > MAX_THRESH) {
    int32_t *lo = base_ptr1;
    int32_t *hi = lo + (nX - 1);
...
#define MIN(X,Y) ((X) < (Y) ? (X) : (Y))
  ...
  {
    int32_t *end_ptr = base_ptr1 + (nX - 1);
    int32_t *run_ptr;
    int32_t *tmp_ptr = base_ptr1;
    int32_t *thresh  = MIN (end_ptr, base_ptr1 + max_thresh);
    ...
    if (tmp_ptr != base_ptr1) {
      SWAP (tmp_ptr, base_ptr1);
    }

    /* Insertion sort, running from left-hand-side up to `right-hand-side.' 
       Pretty much straight out of the original GNU qsort routine. */

    for (run_ptr = base_ptr1 + size; 
