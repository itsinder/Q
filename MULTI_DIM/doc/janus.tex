...
\label{sumby}
\begin{itemize}
\item \verb+w = Q.sumby(x, y, ny, z)+ 
\item \(x\) is a numeric vector of length \(n\)
\item \(y\) is a numeric vector of length \(n\)
\item \(ny\) is a number such that 
\(0 \leq \mathrm{min}(y) \leq \mathrm{max}(y) \leq ny-1\)
...
\begin{quote}
Those who don't remember the past are condemned to repeat it\footnote{GHeorge
Santayana}
\end{quote}
